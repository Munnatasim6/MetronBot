  ...
  return (
    <div className='h-full w-full bg-gray-950 p-4 animate-fade-in overflow-y-auto custom-scrollbar'>
      <div className='flex flex-col lg:flex-row gap-4 min-h-full lg:h-full'>
        {/* 1. Left Column: Metrics & Order Flow */}
        <div className='w-full lg:w-72 flex flex-col gap-4 shrink-0'>
          ...
          <div className='grid grid-cols-2 gap-4 shrink-0'>
            ...
            <div className='bg-gray-900/50 border border-gray-800 rounded-xl p-3 shadow-lg backdrop-blur-sm flex flex-col justify-center items-center'>
              ...
              <div
                className={`text-xl font-bold font-mono ${metrics.timeOffset > 500 ? 'text-red-500' : 'text-green-500'}`}
              >
              ...
              <table className='w-full text-left border-collapse'>
                <thead className='text-xs text-gray-500 font-mono sticky top-0 bg-gray-900/95 backdrop-blur z-10 shadow-sm'>
                  <tr>
                    ...
                    <th className='pb-3'>
                      SIZE
                      <span
                        className='ml-1 text-[9px] px-1 py-0.5 rounded bg-gray-800 text-gray-400 font-normal cursor-help border border-gray-700'
                        title={`Sizing Mode: ${riskConfig.sizingMode}`}
                      >
                ...
                <tbody className='font-mono text-sm'>
                  {positions.map((pos) => (
                    <tr
                      key={pos.id}
                      className='border-b border-gray-800/50 group hover:bg-gray-800/30 transition-colors'
                    >
                      <td className='py-3 pl-2 text-white font-bold'>{pos.pair}</td>
                      <td
                        className={`py-3 ${pos.side === 'LONG' ? 'text-green-500' : 'text-red-500'}`}
                      >
                        {pos.side}
                      </td>
                      <td className='py-3 text-gray-400'>{pos.size}</td>
                      <td
                        className={`py-3 font-bold ${pos.pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}
                      >
                        {pos.pnl >= 0 ? '+' : ''}
                        {pos.pnl.toFixed(2)}
                      </td>
                          ...
                          <ResponsiveContainer width='100%' height='100%'>
                            <LineChart data={pos.pnlHistory?.map((v, i) => ({ v, i })) || []}>
                              <Line
                                type='monotone'
                                dataKey='v'
                                stroke={pos.pnl >= 0 ? '#4ade80' : '#f87171'}
                                strokeWidth={2}
                                dot={false}
                      ...
                      <td className='py-3 text-right flex justify-end gap-2 items-center h-full pr-2'>
                        <button
                          ...
                          className='p-1.5 text-gray-600 hover:text-white transition-colors bg-gray-800 hover:bg-gray-700 rounded'
                        ...
                        <button
                          ...
                          className='px-3 py-1 bg-red-500/10 text-red-500 text-xs rounded border border-red-500/20 hover:bg-red-500 hover:text-white transition-colors font-bold'
                        >
                          CLOSE
                        </button>
                      </td>
        ...
        {/* 3. Console Log (Right) */}
        <div className='w-full lg:w-96 bg-black border border-gray-800 rounded-xl p-4 shadow-lg flex flex-col shrink-0 min-h-[300px] lg:h-full'>
              ...
              >
                ...
                <span
                  className={`
                   ...
                   ${log.type === 'TRADE' ? 'text-neon-green' : ''}
                   ${log.type === 'ERROR' ? 'text-red-500' : ''}
                   ${log.type === 'ALERT' ? 'text-red-500 font-bold bg-red-500/10 px-1 rounded' : ''}
                `}
                >
                  {log.type === 'TRADE' && '$ '}
                  {log.message}
